...
public class SSView extends AbstractSpongeCommand
{
    public SSView()
    {
        super(Commands.VIEW_NAME, Commands.VIEW_DESC, new SpongeCommandUsage(Arrays.asList(new SpongeCommandArgument(Commands.SS_CMD), new SpongeCommandArgument(Commands.VIEW_NAME), new SpongeCommandArgument(Commands.PLAYER, Syntax.OPTIONAL, Syntax.REPLACE), new SpongeCommandArgument(Commands.BUILD, Syntax.OPTIONAL, Syntax.REPLACE))), new SpongeCommandPermissions(Permissions.VIEW, true));
    }
    ...
    public CommandResult process(@Nonnull CommandSource source, @Nonnull String arguments)//NOSONAR
    {
        ...
        if (args.length > 0)
        {
            ...
            SpongeServerSaturday.instance().getSubmissions().openMenu(1, player.getUniqueId());
