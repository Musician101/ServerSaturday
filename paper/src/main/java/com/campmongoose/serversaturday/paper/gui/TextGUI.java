...
public class TextGUI {
    ...
    private static Component getAllComponent(Submitter submitter, Build build) {
        return text(build.getName(), GOLD).hoverEvent(HoverEvent.showText(text("by " + PaperSubmissionsUtil.getSubmitterName(submitter), fromHexString("#BDB76B")))).clickEvent(callback(a -> BuildGUI.open(build, submitter, (Player) a)));
    }
    ...
    private static Component getAllComponent(Submitter submitter) {
        return text(PaperSubmissionsUtil.getSubmitterName(submitter), GOLD).hoverEvent(HoverEvent.showText(text(submitter.getBuilds().size() + " builds.", fromHexString("#BDB76B")))).clickEvent(callback(a -> displaySubmitter((Player) a, submitter, 1)));
    }
    ...
    public static void displaySubmitter(@NotNull Player player, @NotNull Submitter submitter, int page) {
        display(PaperSubmissionsUtil.getSubmitterName(submitter) + "'s Builds", player, page, submitter.getBuilds().stream().map(b -> getViewComponent(submitter, b)).collect(Collectors.toList()), (p, i) -> displaySubmitter(p, submitter, i));
    }
}
