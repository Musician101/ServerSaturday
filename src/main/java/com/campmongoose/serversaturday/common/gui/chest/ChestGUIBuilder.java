...
public abstract class ChestGUIBuilder<B extends ChestGUIBuilder<B, C, G, I, P, S, T>, C, G extends ChestGUI<C, I, P, S>, I, P, S, T> implements Builder<B, G> {
    ...
    protected final List<GUIButton<C, S>> buttons = new ArrayList<>();
    ...
    @Nonnull
    public final B setButton(@Nonnull GUIButton<C, S> button) {
    ...
    @Nonnull
    public final <O> B setContents(@Nonnull C clickType, @Nonnull List<O> contents, @Nonnull Function<O, S> itemStackMapper, @Nullable Function<O, Runnable> actionMapper) {
        int size = this.size - 9;
        for (int x = 0; x < size; x++) {
            try {
                O content = contents.get(x + (page - 1) * size);
                S itemStack = itemStackMapper.apply(content);
                setButton(new GUIButton<>(x, clickType, itemStack, actionMapper == null ? null : actionMapper.apply(content)));
            }
    ...
    @Nonnull
    public abstract B setJumpToPage(int slot, int maxPage, @Nonnull Consumer<Integer> action);
    ...
    @Nonnull
    public abstract B setPageNavigation(int slot, @Nonnull T name, @Nonnull Runnable action);
