...
public class ForgeRewardGiver extends RewardGiver<PlayerLoggedInEvent, ServerPlayerEntity> {
    ...
    public void onJoin(@Nonnull PlayerLoggedInEvent event) {
        ...
        if (rewardsWaiting.getOrDefault(uuid, 0) > 0) {
            MinecraftServer server = LogicalSidedProvider.INSTANCE.get(LogicalSide.SERVER);
            server.enqueue(new TickDelayedTask(server.getTickCounter() + 20, () -> player.sendMessage(new StringTextComponent(Messages.REWARDS_WAITING).setStyle(Style.EMPTY.setFormatting(TextFormatting.GOLD)))));
        }
    }
