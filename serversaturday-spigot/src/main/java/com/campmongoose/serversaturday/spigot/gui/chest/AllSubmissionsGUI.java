...
public class AllSubmissionsGUI extends SpigotChestGUI {
    ...
    private void updateSlots() {
        ...
        SpigotServerSaturday.instance().getSubmissions().getSubmitters().forEach(submitter -> submitter.getBuilds().stream().filter(build -> !build.featured() && build.submitted()).forEach(build -> map.put(build, submitter)));
        List<SpigotBuild> builds = new ArrayList<>(map.keys());
        IntStream.of(0, 45).forEach(x -> {
            try {
                int index = x + (page - 1) * 45;
                SpigotBuild build = builds.get(index);
                SpigotSubmitter submitter = map.get(build).iterator().next();
